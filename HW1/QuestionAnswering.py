...
# ROOT_PATH = 'drive/MyDrive/Colab Notebooks/ADL/HW1/'
...
ROOT_PATH = "./" # comment out if using colab
# manual args, Comment out for running in terminal
TRAIN_PATH = ROOT_PATH + "./training_test.json"
VAL_PATH = ROOT_PATH + "valid.json"
CONTEXT_PATH = ROOT_PATH + "context.json"
OUTPUT_PATH = ROOT_PATH + "OUTPUTS/temp/output_QA"
str_args = [
...
def parse_args(str_args):
    ...
    parser.add_argument(
        "--context_file", type=str, default=None, help="context"
    )
    parser.add_argument(
...
def postprocess_qa_predictions(
    ...
    if output_dir is not None:
        ...
        with open(prediction_file, "w",  errors='ignore') as writer:
            writer.write(json.dumps(all_predictions, indent=4, ensure_ascii=False) + "\n")
        logger.info(f"Saving nbest_preds to {nbest_file}.")
        with open(nbest_file, "w",  errors='ignore') as writer:
            writer.write(json.dumps(all_nbest_json, indent=4, ensure_ascii=False) + "\n")
        if version_2_with_negative:
            logger.info(f"Saving null_odds to {null_odds_file}.")
            with open(null_odds_file, "w",  errors='ignore') as writer:
...
def main(str_args = None):
    ...
    def prepare_train_features(examples):
        ...
        examples[context_column_name] = [CONTEXT_FILE[id] for id in examples[context_column_name]]
...
# * 'references': {'id': Value(dtype='string', id=None), 'answers': Sequence(feature={'text':
# Value(dtype='string', id=None), 'answer_start': Value(dtype='int32', id=None)}, length=-1, id=None)}},

# %%
from accelerate import notebook_launcher
if __name__ == "__main__":
    if USE_NOTEBOOK_LAUNCHER == True:
